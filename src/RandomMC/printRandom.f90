  ...
  subroutine printRandom ( lineVerbose, lineType, lineStr )
...
! ======================================================================
    ...
    select case( j )
       ...
       case ( erprnt_misc    );   j = lineType
       case default;              j = erprnt_fatal   ! Assume fatal error for bad values of (lineType)
    end select

    
    if ( j == erprnt_fatal .or. rngVerbose >= lineVerbose ) then
