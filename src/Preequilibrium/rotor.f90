  ...
  subroutine rotor (preeqObj, ar, br, pstar, pr)
...
!  The cross product is not exactly 0 to machine precision:
       if (cmod < div0Lim .and. cmod > -div0Lim) then
          cmod = div0Lim
          write(preeqObj%io%message,1100) "81"
          call preeqObj%io%print(4, 3, preeqObj%io%message)
       end if
       fac = one/cmod
       do ir = 1,3
...
!       write (16,1000) cp, alpha2, (ar(ir),ir=1,3), (br(ir),ir=1,3), &
!            & (cn(ir),ir=1,3), (pr(ir),ir=1,3)
    endif
...
! 1000 format (2x,'NEW ROTOR; cp = ',1pe15.7,', sp = ',e15.7, &
!           & /2x,'ar = ',3e15.7/2x,'br = ',3e15.7, &
!           & /2x,'cn = ',3e15.7/2x,'pr = ',3e15.7)
1100 format ("Divide by zero error prevented in 'rotor.f90', line ", A)
! ======================================================================
